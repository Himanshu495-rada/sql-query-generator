...
interface Message {
  ...
  sql?: string;
  timestamp: Date;
  queryData?: ApiQueryResponseData;
  isError?: boolean;
}
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (templateType === "generate-sql") {
      initialText =
        "I'm ready to help you generate SQL from text. Describe what you need!";
      ...
      initialText =
        "Need an SQL query explained? Paste it here, and I'll break it down for you.";
    }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response: GenerateQueryApiResponse =
        await queryService.generateQuery(payload);
      ...
      if (response.data?.query) {
        ...
        if (aiResponse.sqlQuery && typeof aiResponse.sqlQuery === "string") {
          ...
          if (jsonStringInSql.startsWith("```json")) {
            jsonStringInSql = jsonStringInSql
              .replace(/^```json\s*/, "")
              .replace(/\s*```$/, "");
          ...
          try {
            const processedForJsonParse = jsonStringInSql.replace(
              /"\s*\+\s*"/g,
              ""
            );
            const parsedSqlData: ParsedSqlData = JSON.parse(
              processedForJsonParse
            );
            finalSql = parsedSqlData.query || "";
            finalExplanation =
              parsedSqlData.explanation ||
              aiResponse.explanation ||
              "No detailed explanation provided.";
          } catch (parseError) {
            console.warn(
              "ChatPage: Failed to parse inner JSON from sqlQuery, attempting fallback:",
              parseError
            );
            const sqlMatch = aiResponse.sqlQuery.match(
              /```(?:sql)?\s*([\s\S]*?)\s*```/
            );
            if (sqlMatch && sqlMatch[1]) {
              finalSql = sqlMatch[1].trim();
            } else if (
              !aiResponse.sqlQuery.includes("json") &&
              !aiResponse.sqlQuery.includes("{")
            ) {
            ...
            finalExplanation =
              aiResponse.explanation ||
              "Could not parse SQL details, using original explanation if available.";
            if (!finalSql && !jsonStringInSql.includes("{"))
              finalSql = jsonStringInSql;
          }
        ...
        throw new Error(
          "Failed to generate SQL query or response structure is not as expected."
        );
      }
    } catch (error: unknown) {
          ...
          <div className={styles.inputSection}>
            <div className={styles.inputControls}>
              ...
              <button
                ...
                disabled={
                  !inputValue.trim() || !selectedDatabaseId || isLoading
                }
              >
                Send
              </button>
