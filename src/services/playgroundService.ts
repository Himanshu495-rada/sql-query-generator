...
class PlaygroundService {
  ...
  async getPlaygrounds(): Promise<Playground[]> {
    try {
      ...
      if (
        ...
        response?.data?.playgrounds &&
        Array.isArray(response.data.playgrounds)
      ) {
        // Format: {success: true, data: {playgrounds: [...]}}
        playgroundsData = response.data.playgrounds;
        console.log(
          "Found playgrounds in success.data.playgrounds format",
          playgroundsData
        );
      }
      // Check other possible formats
        ...
        response?.data?.data?.playgrounds
      ) {
      ...
      if (
        playgroundsData &&
        Array.isArray(playgroundsData) &&
        playgroundsData.length > 0
      ) {
        console.log(
      ...
      console.warn(
        "API did not return valid playground data, returning empty array"
      );
      return [];
    } catch (error) {
   ...
   */
  async addConnectionToPlayground(
    playgroundId: string,
    connectionId: string
  ): Promise<Playground> {
    try {
      const response = await api.post(
        `playgrounds/${playgroundId}/connections`,
        { connectionId }
      );
